#!/usr/bin/env python3
"""
Enhanced SPY Strategy with Improved Entry Detection
Better captures the dip entries and uptrends shown in original screenshots
"""
...
def run_enhanced_backtest():
    """Run enhanced SPY strategy with improved entry detection."""
    print("🎯 Enhanced SPY Multi-Timeframe Strategy")
    print("=" * 60)
    print("Improved entry detection to better capture dips and uptrends")

    # Configuration with more sensitive parameters
    config = SPYMultiTimeframeConfig(
        polygon_api_key=None,
        risk_per_trade=0.01,
        max_portfolio_heat=0.20,
        deviation_std=1.2  # Slightly tighter for more signals
    )
    ...
    print("\n📊 Implementing enhanced multi-timeframe strategy...")

    # Step 1: Find 4-hour dips in bullish daily regime
    four_hour_bullish = four_hour_ema_9 > four_hour_ema_20

    # More sensitive dip detection - near OR below lower band
    four_hour_dips = four_hour_data['close'] <= four_hour_lower_dev * 1.05  # Near or below lower band
    ...
    # Track last trade time to prevent over-trading
    last_trade_time = None
    min_trade_interval = pd.Timedelta(hours=12)  # Minimum 12 hours between trades
        ...
        if four_hour_dips.loc[four_hour_time] and four_hour_bullish.loc[four_hour_time]:
            ...
            if daily_date in daily_data.index and bullish_regime.loc[daily_date]:
                ...
                if last_trade_time is None or (four_hour_time - last_trade_time) >= min_trade_interval:
                    ...
                    # Look for hourly entry in next 6-12 hours (wider window)
                    ...
                    for j, (hourly_time, hourly_row) in enumerate(hourly_data[hourly_mask].iterrows()):
                        # Multiple entry criteria for better signal detection
                        score = 0

                        # 1. Price above dip price (recovering)
                        if hourly_row['close'] > four_hour_row['close']:
                            score += 2

                        # 2. Positive momentum (but not too strong - avoid chasing)
                        if j > 0:
                            prev_close = hourly_data.iloc[hourly_data.index.get_loc(hourly_time) - 1]['close']
                            momentum = (hourly_row['close'] - prev_close) / prev_close
                            if 0.0005 <= momentum <= 0.005:  # Gentle momentum
                                score += 3
                            elif momentum > 0.001:  # Any positive momentum
                                score += 1

                        # 3. Hourly EMAs bullish
                        if hourly_ema_9.loc[hourly_time] > hourly_ema_20.loc[hourly_time]:
                            score += 2

                        # 4. Price above 4-hour EMAs
                        if hourly_row['close'] > four_hour_ema_9.loc[hourly_time]:
                            score += 1

                        # 5. Not too extended from dip
                        price_extension = (hourly_row['close'] - four_hour_row['close']) / four_hour_row['close']
                        if price_extension <= 0.02:  # Within 2% of dip price
                            score += 2

                        # Update best entry if this score is better
                        if score > best_score:
                            best_score = score
                            best_entry = {
                                'time': hourly_time,
                    ...
                    # Execute trade if we found a good entry (minimum score threshold)
                    if best_entry and best_entry['score'] >= 5:
                        ...
                        # Simulate trade execution - look for exit in next 24-72 hours
                        exit_window_end = min(best_entry['time'] + pd.Timedelta(hours=72), hourly_data.index[-1])
                        ...
                        for k, (exit_time, exit_row) in enumerate(hourly_data[exit_mask].iterrows()):
                            ...
                            elif k >= 47:  # Exit after ~48 hours if no target hit
    ...
    if len(trades) > 0:
        ...
        print(f"\n📋 Trade Log:")
        print("-" * 110)
        print(f"{'#':<3} {'Entry':<16} {'Exit':<16} {'Entry $':<8} {'Exit $':<8} {'P&L $':<10} {'P&L %':<8} {'Score':<5} {'Reason':<10}")
        print("-" * 110)
        ...
        for i, trade in enumerate(trades):
            ...
            print(f"{i+1:<3} {entry_str:<16} {exit_str:<16} "
                  ...
                  f"{pnl_str:<10} {pct_str:<8} {score_str:<5} {reason:<10}")
    ...
    # Enhanced multi-timeframe overview chart
    ...
    plt.tight_layout()
    plt.savefig(charts_dir / 'enhanced_strategy.png', dpi=300, bbox_inches='tight')
    print(f"✅ Saved enhanced chart: {charts_dir / 'enhanced_strategy.png'}")
...
def main():
    ...
    print("🎯 Enhanced SPY Multi-Timeframe Strategy")
    print("=" * 60)
    print("This version improves entry detection to better capture:")
    print("1. 4-hour dip identification (more sensitive)")
    print("2. Hourly entry timing (multiple criteria)")
    print("3. Better trade execution and management")
    print("4. More opportunities while maintaining quality")
    ...
    if results:
        ...
        print("• More sensitive dip detection")
        print("• Multi-criteria entry scoring")
        print("• Better trade timing and management")
        print("• Improved opportunity capture")

        print(f"\n📁 Chart saved: backtest_charts/enhanced_strategy.png")
        print(f"📊 Enhanced version should show better trade execution!")

        return results
    else:
        print("❌ Enhanced strategy failed")
        return None
